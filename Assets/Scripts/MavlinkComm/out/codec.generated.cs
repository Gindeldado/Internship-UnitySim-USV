...
namespace MavLink
{
    ...
    public static class MavLinkSerializer
    {
        ...
        public static Dictionary<int, MavPacketInfo> Lookup = new Dictionary<int, MavPacketInfo>
        {
			...
			{332, new MavPacketInfo(Deserialize_TRAJECTORY_REPRESENTATION_WAYPOINTS, 39)},
			...
			{365, new MavPacketInfo(Deserialize_STATUSTEXT_LONG, 36)},
		...
		internal static MavlinkMessage Deserialize_TRAJECTORY_REPRESENTATION_WAYPOINTS(byte[] bytes, int offset)
		{
			return new Msg_trajectory_representation_waypoints
			{
				...
				vel_x =  ByteArrayUtil.ToSingle(bytes, offset + 48, 5),
				vel_y =  ByteArrayUtil.ToSingle(bytes, offset + 68, 5),
				acc_x =  ByteArrayUtil.ToSingle(bytes, offset + 88, 5),
				acc_y =  ByteArrayUtil.ToSingle(bytes, offset + 108, 5),
				pos_yaw =  ByteArrayUtil.ToSingle(bytes, offset + 128, 5),
				vel_yaw =  ByteArrayUtil.ToSingle(bytes, offset + 148, 5),
				accept_radius =  ByteArrayUtil.ToSingle(bytes, offset + 168, 5),
				command =  ByteArrayUtil.ToUInt16(bytes, offset + 188, 5),
				valid_points = bytes[offset + 198],
			};
		}
		...
		internal static MavlinkMessage Deserialize_STATUSTEXT_LONG(byte[] bytes, int offset)
		{
			return new Msg_statustext_long
			{
				severity = bytes[offset + 0],
				text =  ByteArrayUtil.ToChar(bytes, offset + 1, 254),
			};
		}
		...
		internal static int Serialize_TRAJECTORY_REPRESENTATION_WAYPOINTS(this Msg_trajectory_representation_waypoints msg, byte[] bytes, ref int offset)
		{
			...
			ByteArrayUtil.ToByteArray(msg.vel_x, bytes, offset + 48, 5);
			ByteArrayUtil.ToByteArray(msg.vel_y, bytes, offset + 68, 5);
			ByteArrayUtil.ToByteArray(msg.acc_x, bytes, offset + 88, 5);
			ByteArrayUtil.ToByteArray(msg.acc_y, bytes, offset + 108, 5);
			ByteArrayUtil.ToByteArray(msg.pos_yaw, bytes, offset + 128, 5);
			ByteArrayUtil.ToByteArray(msg.vel_yaw, bytes, offset + 148, 5);
			ByteArrayUtil.ToByteArray(msg.accept_radius, bytes, offset + 168, 5);
			ByteArrayUtil.ToByteArray(msg.command, bytes, offset + 188, 5);
			bytes[offset + 198] = msg.valid_points;
			offset += 199;
			return 332;
		}
		...
		internal static int Serialize_STATUSTEXT_LONG(this Msg_statustext_long msg, byte[] bytes, ref int offset)
		{
			bytes[offset + 0] = msg.severity;
			ByteArrayUtil.ToByteArray(msg.text, bytes, offset + 1, 254);
			offset += 255;
			return 365;
		}
